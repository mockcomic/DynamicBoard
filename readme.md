...
- `birthday(name,mm,dd,yyyy,daysAhead,daysToText,dayOfText)`  
  ...
  string. `daysToText` and `dayOfText` are optional custom templates that support
  tokens: `{name}`, `{days}`, `{dayLabel}`.
- `todayDate()`  
